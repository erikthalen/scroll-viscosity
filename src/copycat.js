...
export default {
  ...
  applyStyles(viscosity) {
    ...
    Object.assign(viscosity.copycat.style, {
      ...
      display:
        (!isImage(viscosity.subject) &&
          this._ifNot('list-item', 'block')(display) &&
          display === 'inline' &&
          'inline-block') ||
        '',
      left:
        this._ifNot('static', 'relative')(position) &&
        (position === 'absolute'
  ...
  _getSpacings(viscosity, type) {
    return `${this._getSpacing(viscosity, `${type}Top`)} ${this._getSpacing(
      viscosity,
      `${type}Right`
    )} ${this._getSpacing(viscosity, `${type}Bottom`)} ${this._getSpacing(
      viscosity,
      `${type}Left`
    )}`
  },
  ...
  _getSpacing(viscosity, direction) {
    ...
    return (
      Math.max(
        parseFloat(viscosity.originalPlacement[direction]),
        parseFloat(this._getChildSpacing(viscosity.subject, direction))
      ) + 'px'
    )
  },
  ...
  _getChildSpacing(subject, direction) {
    ...
    if (
      firstOrLastChild &&
      subject[firstOrLastChild] &&
      subject[firstOrLastChild][firstOrLastChild]
    ) {
  ...
  _ifNot(...values) {
    return prop => (values.includes(prop) ? null : prop)
  },
}
